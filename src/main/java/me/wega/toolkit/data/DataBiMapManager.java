...
public abstract class DataBiMapManager<K, V> {
    private BiMap<K, V> dataMap;

    public DataBiMapManager() {
        this.dataMap = HashBiMap.create();
    }

    @SneakyThrows
    public DataBiMapManager(@NotNull Class<? extends BiMap<K, V>> mapType) {
        this();
    ...
    public boolean hasValue(@NotNull V value) {
        return dataMap.inverse().containsKey(value);
    }

    public boolean removeValue(@NotNull V value) {
        return dataMap.inverse().remove(value) != null;
    }

    public K getValueKey(@NotNull V value) {
        return dataMap.inverse().get(value);
    }

    public @NotNull Map<V, K> getInverseMap() {
        return new HashMap<>(dataMap.inverse());
    }
